...
serve(async (req) => {
  ...
  try {
    ...
    const { action, standardId, topic, difficulty, questionCount, examId, questionId, userAnswer } = await req.json();
    const LOVABLE_API_KEY = Deno.env.get("LOVABLE_API_KEY");
    if (!LOVABLE_API_KEY) throw new Error("LOVABLE_API_KEY not configured");

    // ── GENERATE QUIZ QUESTIONS ──
    if (action === "generate_questions") {
      // Fetch chunks from the standard
      ...
      const aiResponse = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: `You are an exam question generator for trade apprentices studying ${standard?.title || "industry standards"}. Generate multiple-choice questions ONLY from the provided standard content. Never invent facts. Each question must reference specific clauses.`,
            },
            {
              role: "user",
              content: `Generate ${count} ${diff}-difficulty multiple-choice questions from this standard content. ${topicFilter}
...
${chunks.map((c) => `[${c.clause_number || ""}] ${c.content}`).join("\n\n")}`,
    ...
    if (action === "start_exam") {
      const timeLimit = 30 * 60; // 30 min default
      ...
      return new Response(JSON.stringify({ exam, questions: shuffled }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (action === "submit_answer") {
      const { data: question } = await supabase
        .from("capstone_questions")
        .select("correct_answer, explanation, clause_reference")
        .eq("id", questionId)
        .single();
      ...
      if (isCorrect) {
        await supabase.rpc("increment_correct_answers" as any, { exam_id_param: examId }).catch(() => {
          // Fallback: manual update
          supabase.from("capstone_exams").update({ correct_answers: 0 }).eq("id", examId);
        });
      }

      return new Response(
        JSON.stringify({ is_correct: isCorrect, correct_answer: question.correct_answer, explanation: question.explanation, clause_reference: question.clause_reference }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    if (action === "complete_exam") {
      ...
      await supabase
        .from("capstone_exams")
        .update({ status: "completed", correct_answers: correct, total_questions: total, completed_at: new Date().toISOString() })
        .eq("id", examId);

      const percentage = total > 0 ? Math.round((correct / total) * 100) : 0;
      const passed = percentage >= 70;

      return new Response(JSON.stringify({ correct, total, percentage, passed }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
      ...
      const aiResponse = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "You are an expert trade educator. Create concise, apprentice-friendly study guides from standard content. Use clear headings, bullet points, and highlight key clause numbers. Only use information from the provided content.",
            },
            {
              role: "user",
              content: `Create a comprehensive study guide for apprentices from this standard.${topic ? ` Focus on: ${topic}` : ""}
